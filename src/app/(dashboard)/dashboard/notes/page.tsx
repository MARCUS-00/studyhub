...
export default function NotesPage() {
  ...
  const allNotes = useAppSelector(NotesSelector.selectAll).filter(
    (n) => !n.limit_visibility,
  );
  useEffect(() => {
    if (allNotes.length === 0) dispatch(getNotes());
  ...
  const filtered = useMemo(() => {
    ...
    return allNotes.filter((n) => {
      ...
      const matchSub =
        subFilter === "all" || n.subjects?.sub_code === subFilter;
  ...
  const inputCls =
    "rounded-xl border border-forest/15 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald/40 focus:border-emerald transition";
